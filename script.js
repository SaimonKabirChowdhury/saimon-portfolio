...
/* Tilt is desktop-only. On iPad/phones, pointermove can compete with native swipe scrolling. */
...
function resize(){
  ...
  particles = Array.from({length: Math.min(88, Math.floor(innerWidth / 16))}, () => ({
    x:Math.random()*innerWidth,
    y:Math.random()*innerHeight,
    r:Math.random()*1.6+.3,
    v:Math.random()*.28+.06,
    a:Math.random()*.42+.08,
    c:Math.random()>.76?'255,106,36':'117,216,255'
  }));
}
